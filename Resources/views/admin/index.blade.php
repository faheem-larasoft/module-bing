            ...
            <div class="margin">
                ...
                <div style="width: 100%; float:left;" class="admin">
                    ...
                    <table class="formatted" id="datatable">
                        <thead>
                        <tr class="header">
                            <td colspan="8"><h3>Campaign</h3></td>
                        ...
                        @foreach($campaigns as $campaign)
                            <tr @if($campaign->status != 'Active') class="greyed" @endif>
                                <td>
                                    <input type="checkbox" name="campaigns[{{ $campaign->id }}]" value="{{ $campaign->id }}" style="margin:0px;">
                                </td>
                                <td>
                                    <a href="{{ route('admin_bing_campaign', $campaign->id) }}">{{ $campaign->name }}</a>
                                </td>
                                <td data-sort="{{ $campaign->status == 'Active' ? '0' : '1' }}">
                                ...
                                <td>
                                    {{ number_format($spend->where('campaign_id', $campaign->id)->sum('clicks')) }}
                                </td>
                                <td>
                                    {{ number_format($spend->where('campaign_id', $campaign->id)->sum('impressions')) }}
                                </td>
                                <td>
                                    {{ number_format($spend->where('campaign_id', $campaign->id)->sum('impressions') > 0 ? 100*$spend->where('campaign_id', $campaign->id)->sum('clicks')/$spend->where('campaign_id', $campaign->id)->sum('impressions') : 0, 1) }}%
                                </td>
                                <td>
                                    £{{ number_format($spend->where('campaign_id', $campaign->id)->sum('clicks') > 0 ? $spend->where('campaign_id', $campaign->id)->sum('cost')/$spend->where('campaign_id', $campaign->id)->sum('clicks')/100 : 0, 2) }}
                                </td>
                                <td>
                                    £{{ number_format($spend->where('campaign_id', $campaign->id)->sum('cost')/100, 2) }}
                                </td>
                                <td>
                                    {{ number_format($spend->where('campaign_id', $campaign->id)->sum('clicks') > 0 ? $statistics->where('campaign_id', $campaign->id)->sum('quotes.total')/$spend->where('campaign_id', $campaign->id)->sum('clicks')*100 : 0, 1) }}%
                                </td>
                                <td>
                                    {{ number_format($statistics->where('campaign_id', $campaign->id)->sum('quotes.total')) }}
                                </td>
                                <td>
                                    {{ number_format($statistics->where('campaign_id', $campaign->id)->sum('tradespeople.total')) }}
                                </td>

                                <td>
                                    {{ number_format($statistics->where('campaign_id', $campaign->id)->sum('tradespeople.verified')) }}
                                </td>
                                <td>
                                    {{ number_format($statistics->where('campaign_id', $campaign->id)->sum('tradespeople.total') > 0 ? $statistics->where('campaign_id', $campaign->id)->sum('tradespeople.verified')/$statistics->where('campaign_id', $campaign->id)->sum('tradespeople.total')*100 : 0, 1) }}%
                                </td>
                                <td>
                                    {{ number_format($statistics->where('campaign_id', $campaign->id)->sum('tradespeople.haspaid')) }}
                                </td>
                                <td>
                                    {{ number_format($statistics->where('campaign_id', $campaign->id)->sum('tradespeople.hashad')) }}
                                </td>

                                <td>
                                    {{ number_format($statistics->where('campaign_id', $campaign->id)->sum('quotes.priced.1')) }}
                                </td>
                                <td>
                                    {{ number_format($statistics->where('campaign_id', $campaign->id)->sum('quotes.sold')) }}
                                </td>
                                <td>
                                    {{ number_format($statistics->where('campaign_id', $campaign->id)->sum('commission.internal.sales')) }}
                                </td>
                                <td>
                                    &pound;{{ number_format($statistics->where('campaign_id', $campaign->id)->sum('commission.external'), 2) }}
                                </td>
                                <td>
                                    &pound;{{ number_format($statistics->where('campaign_id', $campaign->id)->sum('commission.internal.cost'), 2) }}
                                </td>
                                <td @if($statistics->where('campaign_id', $campaign->id)->sum('commission.total.cost') > $spend->where('campaign_id', $campaign->id)->sum('cost')/100) class="green" @elseif($spend->where('campaign_id', $campaign->id)->sum('cost') > 0 ) class="red" @endif>
                                    {{ returnOnInvestment($statistics->where('campaign_id', $campaign->id)->sum('commission.total.cost'), $spend->where('campaign_id', $campaign->id)->sum('cost')/100) }}%
                                </td>
                                <td @if($statistics->where('campaign_id', $campaign->id)->sum('commission.total.cost') > $spend->where('campaign_id', $campaign->id)->sum('cost')/100) class="green" @elseif($spend->where('campaign_id', $campaign->id)->sum('cost') > 0 ) class="red" @endif>
                                    £{{ number_format($statistics->where('campaign_id', $campaign->id)->sum('commission.total.cost')-($spend->where('campaign_id', $campaign->id)->sum('cost')/100), 2) }}
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
